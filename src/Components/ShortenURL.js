...
function ShortenURL(isMobile) {
        ...
        try {
            ...
            if (!data.ok || data.error) {
                setAlert({
                    ok: false,
                    show: true,
                    msg: data.error || "There was an error",
                })
                return
            } else {
    ...
    return (
        <div className="flex flex-col items-center justify-center lg:w-3/4 h-auto sm:w-full relative -mt-12">
            ...
            >
                <input
                    className="bg-white text-gray-800 placeholder:text-gray-400 py-2 lg:px-6 sm:px-2 h-10 lg:w-2/3 sm:w-full md:w-full rounded-md sm:mb-5 md:mb-auto"
                ...
                <p className="text-white cursor-pointer" onClick={clear}>
                    Clear
                </p>
            </form>
            {alert.show && (
                <p className={`${alert.ok ? "text-teal-500" : "text-red-500"}`}>
                    {alert.msg}
                </p>
            )}
